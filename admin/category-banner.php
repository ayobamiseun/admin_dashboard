<?php
  ...
  if(!isset($_SESSION['GoLiveBlogUser'])){
    header('location:login?ref=category-banner');
  }
  else{
...
<head>
    ...
    <title>Category Banner | GoLive Blog</title>
  ...
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.3.0/codemirror.min.css">-->
    ...
    #upload_overlay {position:fixed; width:100%; height:100%; background:rgba(0,0,0,0.8); z-index:999939999999; display:none; overflow:auto;}
      #upload_con {position:relative; width:1000px; height:auto; padding:10px 0px; background:; margin:70px auto; 
      ...
      <div class="modal-body">
          <div class="form-bg" style="width: 100%;">
              ...
              <div class="form-group help" style="">
                  <input type="hidden" name="type" value="addCatbanner" />
                        ...
                        <div class="">
                            <form id="editform" class="form-horizontal" style="">
                                ...
                                <div class="form-group help" style="">
                                     ...
                                     <input type="hidden" name="type" value="editCatBanner">
                                     <input type="hidden" name="uid" id="uidE">

                                    <label for=""><strong>Title</strong></label>
                                    <input name="title" id="titleE" class="form-control"/>

                                    <label for=""><strong>Post link</strong></label>
                                    <input name="link" id="linkE" class="form-control"/>
                                    ...
                                    <label for=""><strong>Category</strong></label>
                                    <select name="category" class="form-control tt">
                                    <option value="" id="categoryE">Select</option>
                                      <?php 
                                          $sqlc = mysqli_query($con,"select uid,name from category");
                                          while ($rowc = mysqli_fetch_array($sqlc)) {
                                              $uidc = $rowc['uid'];
              ...
              <br />
                ...
                <div class="">
                    ...
                    <div class="">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="header-title">Category banner - <span class="text-primary"> <?php echo $cat; ?> </span>   <button data-toggle="modal" data-target="#modal-1" class="btn btn-primary btn-xs addn" style="margin-left: 10px;">Add new</button></h4>
...
<script src="https://gitcdn.github.io/bootstrap-toggle/2.2.2/js/bootstrap-toggle.min.js"></script>
    ...
    <script type="text/javascript">
      $(document).ready(function() {
       ...
       $('#selart').on('change', function(){
        ...
        $.getJSON('j_cat_banner.php',{category:category}, function(data) {
          $('.MClist').html('');
          ...
          $.each(data.Feed, function(i,result){
            ...
            var newRow =               
                                         ...
                                         '<tr class="clm" data-status="'+result.status+'" >'
                                          ...
                                          +'<td><img src="'+result.image+'" style="width:50px;"/></td>'
                                          +'<td>'+result.title+'</td>'
                                          +'<td>'+result.link+'</td>'
                                           +'<td>'
          ...
          $('.clm').each(function(){
           ...
           $('.delete').click(function(event) {
            ...
            $('.toggle1').change(function(){
              var uid = $(this).attr('data-uid');
              if(this.checked){
                Ajx_live(uid,"on","catbanner");
                //$.toaster({ message : "Enabled",priority : 'info'});
              }
              else{
                Ajx_live(uid,"off","catbanner");
    ...
    function GetOne(uid){
        ...
        $.getJSON('j_cat_banner.php',{uid:uid}, function(data) {
          $('.MCEdit').html('');
          //alert(data.Feed.length);
          $.each(data.Feed, function(i,result){
