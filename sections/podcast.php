<?php
  ...
  if(!isset($_SESSION['SuperFM963'])){
    header('location:login?ref=podcast');
  }
  else{
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        ...
                                        <h4 class="modal-title">Delete this podcast</h4>
                                    </div>
                                    <div class="modal-body">
              ...
              <br />
                ...
                <div class="">
                    ...
                    <div class="">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="header-title">Podcast <span></span>   <button data-toggle="modal" data-target="#modal-11" class="btn btn-primary btn-xs addn" style="margin-left: 10px;">Add new</button></h4>
                                                ...
                                                <div class="">
                                                    <form id="fpost" class="form-horizontal" style="padding-bottom: 0px; overflow-x: hidden;">
                                                        ...
                                                        <div class="form-group help" style="">
                                                            <input type="hidden" name="type" value="addPodcast" />
                                                            ...
                                                            <input type="text" id="track" name="title" class="form-control tt">

                                                            <label>Guest name :  </label>
                                                            <input type="text" id="guest" name="guest" class="form-control tt">

                                                            <label>Audio file :  </label>
                                                            <input type="file" id="link" name="file" class="form-control">

                                                            <label>Category : </label>
                                                            <select name="cat" class="form-control tt">
                                                                <option value="">Select</option>
                                                                <?php 
                                                                  $sqlv = mysqli_query($con,"select * from podcast_cat");
                                                          ...
                                                          <select name="download" class="form-control tt">
                                                            ...
                                                            <label for=""><strong>About podcast / guest</strong></label>
                                                             <div id="editor" style="">
...
<script src="https://gitcdn.github.io/bootstrap-toggle/2.2.2/js/bootstrap-toggle.min.js"></script>
  ...
  <script>
    ...
    <script type="text/javascript">
      $(document).ready(function() {
       ...
       $('#selart').on('change', function(){
        ...
        $.getJSON('j_podcasts.php', function(data) {
          $('.MClist').html('');
         //alert(data.Feed.length);
          $.each(data.Feed, function(i,result){
   

             //PostFireBlog(result.uid,result.odr,result.title,result.thumb,result.details,result.link,result.type,result.status,result.date);
            ...
            var newRow =               
                                         ...
                                         '<tr class="clm" data-status="'+result.status+'" >'
                                          ...
                                          +'<td>'+result.title+'<p><a target="_blank" href="https://superfm963.com/podcast?id='+result.uid+'">https://superfm963.com/podcast?id='+result.uid+'</a></p></td>'
                                          +'<td><button class="btn btn-info btn-xs">'+result.cat+'</button></td>'
                                          +'<td><button class="btn btn-success btn-xs">'+result.view+'</button></td>'
                                          +'<td>'+result.date+'</td>'
                                          +'<td>'
                                              +'<a href="edit-podcast.php?uid='+result.uid+'" class="btn btn-primary btn-xs edit1" data-uid="'+result.uid+'" style="margin:0px 5px;">Edit</a>'
          ...
          $('.clm').each(function(){
           ...
           $('.delete').click(function(event) {
            ...
            $('.toggle1').change(function(){
              var uid = $(this).attr('data-uid');
              if(this.checked){
                Ajx_live(uid,"on","music");
                //$.toaster({ message : "Enabled",priority : 'info'});
              }
              else{
                Ajx_live(uid,"off","music");
    ...
    function GetOne(uid){
        ...
        $.getJSON('j_posts.php',{uid:uid, tt:"title"}, function(data) {
          $('.MCEdit').html('');
